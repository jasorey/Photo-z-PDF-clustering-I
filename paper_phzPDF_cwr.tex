...
of the major probes to understand the history of the Universe and the growth of
...
structures. However, targeting galaxies and obtaining spectra is a slow and costly process
and therefore past and current spectroscopic surveys have been limited to lower
redshifts and a reduced number of galaxies.
...
Synoptic Survey Telescope (LSST\footnote{http://www.lsst.org/lsst/}, \pcite{lsstcol}) can extract redshift information 
...
Photometric redshifts (photo-z) are obtained using multi-band photometry in combination with different techniques which
translate the photometric properties of the galaxies into a redshift. One broad category of these include template-based
...
The other techniques use a spectroscopic training set and machine learning 
...
Probability distribution functions of various astronomical measurements have been used in cosmological analyses, 
for example luminosity functions \cite{sheth07} weak lensing \cite{mandelbaum2008}, cluster selection techniques 
\cite{vanbreukelen09}, and real-space clustering of quasars \cite{myers09}. 
...
In this work, we study how galaxy angular clustering depends on the chosen photo-$z$ estimate and on the photo-$z$ bin width,
by comparing with respect to true redshift clustering, using realistic simulations. We also show how to include the full probability distribution when 
estimating the angular correlation functions. We study the impact of using 
different statistical quantities defining the photo-z on the determination of galaxy bias estimation. 
In section \ref{sec:methodology} we describe the methodology followed in the paper. We describe the angular clustering 
results and the fitting to galaxy bias in section \ref{sec:results}, and we discuss them in section \ref{sec:discussion}. 
Finally, we summarize the main conclusions in section \ref{sec:conclusion}.

\section{Methodology}\label{sec:methodology}
The standard approach for analysing galaxy clustering in photometric surveys consists on subdividing the 
catalogue into sub-samples selected by 'top-hat' photo-z redshift bins. 
The value of the photo-z for a galaxy to be in one bin or another is usually a point estimate of this quantity.
The scope of this study is to quantify how angular clustering analysis is affected by the 
choice of the photometric redshift estimator, including one that uses the full PDF information instead of 
photo-z point estimates. We addressed this by
...
redshift estimators when considering different redshift bin configurations. Finally,
we quantified those differences by fitting theoretical galaxy correlation functions
...
Cosmology Challenge (BCC) simulations, developed for DES.  
...
The full BCC-Aardvark-v1.0c catalogue covers 10313 sq. degree to full DES depth, 
for a total of $1.36 \cdot 10^{9}$ galaxies. The simulated catalogue is stored in files 
according to \healpix \footnote{http://healpix.jpl.nasa.gov/} \cite{healpix} pixels of $n_{side}=8$. 
We chose a contiguous area of the simulation by using $24$ of such pixels, 
which corresponds to an area of about $1200$ square degrees on the sphere in order to 
have a significant amount of small scales sampling.
 For our study, we have selected the galaxy sample according to a magnitude 
selection cut of $g<24$. This cut corresponds to a selection in g band of signal-to-noise greater than 20 
...
We have considered $10^5$ galaxies as a training set, up to full depth, therefore using less than $1\%$ 
of the available data for this purpose. The training set galaxies were confined in a region of $54$ square degrees.

As defined in \pcite{tpz}, the concentration of individual galaxy 
PDFs, $p(z)$ is enclosed in TPZ  as an output a PDF concentration parameter called zConf. 
...
selected $\sigma_{TPZ}=0.075$, which is similar to the $1-\sigma$ confidence interval of the PDFs. We can select different quality cuts using this parameter. It is 
related with the BPZ ODDS parameter \cite{BPZ}. 
...
TPZ is a particular method of the MLZ framework. MLZ incorporates a Bayesian combination of techniques that estimate photometric PDFs, such as 
template based methods or unsupervised machine learning algorithms, \cite{bayesian} and 
the possibility to efficiently store the PDFs in the sparse representation basis, \cite{sparse}.  
We only used the particular case of TPZ for the photometric redshifts in this paper given the good results produced by 
TPZ on the DES Science Verification Data \cite{sanchez14} and the fact that we want to study the impact of using 
photo-$z$ PDF in clustering as produced by a single technique. TPZ has been used, together with other codes listed in \pcite{sanchez14}, in 
...
We considered the redshift range $0.2<z<1.4$. 
Galaxy clustering analysis in photometric surveys are usually done measuring angular correlations of 
galaxy samples selected in different redshift bins. We divide the redshift information into 
...
are given by shells of about twice the size of the photometric redshift standard deviation. In this case, we considered different configurations
in this study: $\Delta z=\{0.1,0.15,0.2,0.3\}$ in order to study the evolution of photometric clustering 
with the bin width. The true redshift distribution of galaxies, n(z), is shown in Figure \ref{fig:fig2}, together with the redshift distribution 
obtained by stacking TPZ PDFs. We also show (red) the n(z) of the considered training set.
\begin{figure}
\begin{center}
...
\caption{In blue, the galaxy redshift distribution for 31 million galaxies with $g<24$ in the BCC Aadvark 1.0 catalogue
 in the selected region of $1200$ sq. degrees. The dashed black line shows the 
 result of stacking the individual PDFs of all the galaxies in the sample. In red, we show the true n(z) for the galaxies on the training set used in 
...
\end{center}
\end{figure}
For the study covered in this paper, we were only interested in a patch of the sky that covers $1200$ square 
...
\label{eq:sigmaphz}
\end{equation} 
where $\bar{z}$ is the mean redshift, defined in Equation (\ref{eq:mean}) below, and we compare it with the different redshift bin widths that we have considered in this paper. Although the optimal choice would be $\Delta z=0.15$ or $0.2$, we also consider extreme cases with $\Delta z=0.1$ or $0.3$ to extend the analysis of the dependence of photo-z clustering. In Figure \ref{fig:fig3b}, we show the normalized dispersion between the true redshift and the mean redshift. 
\begin{figure}
\begin{center}
...
Once TPZ obtains the PDF for each galaxy, we can estimate single statistical quantities 
from it. In this study we focused on the mode redshift, $\hat{z}$ and the mean redshift, $\bar{z}$. 

We define the mean as the first momentum of the PDF, $p(z)$:
\begin{equation}
\bar{z}=\int{zp(z)}
\label{eq:mean}
\end{equation}
The mode redshift is the redshift at with highest probability in the PDF $p(z)$
\begin{equation}
p(\hat{z})=p_{max}
\label{eq:mode}
\end{equation}
...
Monte Carlo photo-z is assigned by the redshift that corresponds to the value of the cumulative distribution function given by a random number between (0,1].%It is defined by the redshift that corresponds to the value of the cumulative distribution function given by a random number between (0,1]. 
We also evaluated the median redshift but we decided not to include it in the final results as it is similar to the other point estimates and we decided to 
reduce the analysis to the chosen single point estimates.

We created a catalogue for each redshift bin considered by selecting the galaxies with the single point estimate in the range covered by the redshift bin.
...
for each galaxy in each radial shell, given by the probability that the galaxy lies in the 
...
galaxy clustering using the weights for the galaxy counts. The case that involves the photo-z single 
point estimate cases (mean, mode) is equivalent to setting the weight to  $f_z=1$ for all the galaxies selected 
in the corresponding redshift bin.

We defined threshold cuts, $p_{threshold}$, in a similar way as in \pcite{mandelbaum2008}, 
as the process of considering a galaxy in a redshift bin or not when using weights. Then, a galaxy 
$\alpha$ in redshift bin $j$ would only be incorporated if
\begin{equation}
f_z > p_{threshold}
\label{eq:threscut}
\end{equation}
When PDFs are broad and contain multiple peaks we might be introducing noise on each redshift bin by galaxies that are not in the bin but have 
a non-negligible weight. This can be addressed by applying the threshold cut.

In Figure \ref{fig:fig1}, we show a graphic example of the different photometric redshift estimators that we use. 
We intentionally selected a PDF with a most frequent (mode) redshift, given by eq. (\ref{eq:mode}) within photometric bin $0.5<z<0.8$ but where the mean, defined in eq. (\ref{eq:mean}) is in a different redshift bin. In blue, we show the portion of the PDF between $0.5<z<0.8$ that corresponds to the weight of the galaxy in that redshift bin.
\begin{figure}
\begin{center}
%\includegraphics[trim = 0cm 0cm 0cm 0cm, width=0.43\textwidth]{../Plots/Fig_nz.pdf}
...
\end{center}
\end{figure}
The PDF displayed in th Figure \ref{fig:fig1} is an extreme case. For this particular PDF, zConf=0.37, while near z=0.6 the mean PDF quality parameter is zConf$\sim0.95$.
...
With photo-$z$ PDFs we can easily obtain the photometric sample redshift distribution in each bin $n(z)$ by stacking all the individual $p(z)$ of selected galaxies.
\begin{equation}
N(z)_{pdf}=\sum{p_i(z)f_{z_i}}
\end{equation}
In this paper, we have considered this definition as the default estimation of $n(z)$, setting $f_z=1$ for the galaxies selected according to single point statistics 
in a redsift bin. We can also determine the the true $n(z)$, measured by the distribution of the true redshifts of this simulated sample. Throughout this paper, when we refer to true values, we are considering the latter definition.

\subsection{Two point angular correlation function estimators}
\subsubsection{Pixel based estimator}
We computed the angular correlations using pixel maps of the galaxy density field. These maps are created using {\tt Healpix} for each redshift 
...
the weights of all the galaxies of the pixel, i.e.: $n_i=\sum\limits_{gal \in i}{f_z}$ and $\bar{n}=\sum\limits_i\sum\limits_{gal \in i}{f_z}$.

In order to include errors on our measurements, we considered jackknife samples, dividing the survey area in $N_{JK}$ regions of about $3$ sq. deg. each one. Then, the covariance matrix is given by:
\begin{equation}
C_{\theta_i,\theta_j} = \frac{N_{JK}-1}{N_{JK}}\sum_{k=1}^{N_{JK}}(w_k(\theta_i)-w(\theta_i))(w_k(\theta_j)-w(\theta_j))
\end{equation}
which is the same definition used in \pcite{scranton02,norberg09,wang13}. For the galaxy bias fitting, we considered the mixed approach used in \pcite{crocce15} in which we determine the correlation matrix between diagonal elements and off-diagonal elements of the covariance matrix by using theoretical angular power spectra and finally 
rescaling them with the variances given by the jackknife errors in order to determine the covariance matrix of the angular correlations.	

\subsubsection{Direct pair counting estimator}
An alternative method to measure angular correlations consists of using pair counts estimators. In order to estimate the angular correlation functions, we used the Landy-Szalay estimator, \cite{landyszalay},
\beq
w(\theta)=\frac{DD-2DR+RR}{RR}
\label{eq:wtheta_p2p}
\eeq
where $DD$ is the number of galaxy-galaxy pairs, $DR$ is the galaxy-random pairs and $RR$ the random random pairs within $\theta$ and $\theta + \delta\theta$. The random catalogues are created throwing points in the survey footprint following a uniform density. These are appropriately normalized to the total number of counts. When counting pairs, each galaxy was weighted according to Equation (\ref{eq:weight_d}). We compare in section \ref{sec:results} the point to point estimator with the pixel based estimator.

We computed the angular correlation using the publicly 
available tree code\footnote{http://lcdm.astro.illinois.edu/code/tpacf.html},
explained and used in \pcite{dolence08,wang13}.

\subsection{Theoretical modeling}
The angular auto-correlation at a given redshift bin is given by:
\begin{equation}
...
\label{eq:bias}
\end{equation}
and we define one bias parameter per redshift bin.

We used CAMB, \cite{cambt} to obtain the linear power spectrum and 
Halofit, \cite{halofit,takahashi2012} in order to include non-linearities at small scales. Then, we can Fourier transform it in order to compute the 
3D correlations required by Equation (\ref{eq:wtheta}). For this paper, we considered a flat $\Lambda$CDM model driven by the simulation 
...
\caption{We show a comparison between the pixel based clustering measurement (purple points) and the 
point-to-point clustering measurement (purple shadow include measurement within errorbars) for the photometric redshift bin $1.0<z<1.2$ when considering galaxy weights and 
a threshold on the weights of $f_z>0.1$. We can see the good agreement between these two. }
\label{fig:fig4}
\end{center}	
\end{figure}
...
\end{tabular}
\caption{This Figure shows a comparison between angular clustering measurements using photometric redshifts and true redshifts. We considered different redshift estimators, for redshift bins centered at $z=1$ but different widths. In the top panels we show the results for the mode (red triangles) and the mean (black stars). In the bottom row we show the results when using PDF weights, with threshold $p_{threshold}=0$  (green squares), and when considering thresholds to the weights of $p_{threshold}=0.1$ (purple triangles). We show in each column the corresponding bin width. We show the results for $\hat{z}$ by a red dashed line in order to compare with the upper panels. The ratios with respect to the true redshift results decrease with the bin width and they are different depending on the considered photo-z definition. }%In each panel, we show on the left the two point angular correlation functions for true redshift galaxies and the considered photometric redshift estimators. On the right of each panel, we show the ratio of each photometric redshift sample with respect to the true clustering, categorized in three different subsets. In the topmost, we show the ratios for the mode, $\hat{z}$ and mean, $\bar{z}$. In the middle one, we also show the results for both of them when we apply a $zConf$ cut at $0.8$ for both quantities. We show the results for $\bar{z}$ by a red dashed line in order to compare between plots. Finally, in the bottom plot of each subpanel, we present the ratios for PDF weights samples, with three different thresholds: 0, 0.1 and 0.5.} 
\label{fig:fig5}
\end{center}
\end{figure*}
...
As shown in \pcite{wang13}, the pixel-based and the point-to-point pair count methods yield similar results for 2-point angular clustering. However, it was considering 
full galaxy selections in bins, i.e., weights of $f_z=1$. In Figure \ref{fig:fig4}, we compare the results for both pair count and pixel-based methods when considering galaxy 
weights in the redshift bin $1.0<z<1.2$ and a threshold $p_{threshold}=0.1$ for the angular range $0.1<\theta<1.0$., finding a good agreement. The number of jackknife regions is 
different in the two cases, being $N_{JK}=32$ for the point-to-point case and $N_{JK}=384$ when considering the pixel-based estimator. Given the agreement, we decided to use the pixel-based estimator in the rest of the paper because it is less expensive computationally
...
true redshift clustering. This allows us to study directly how the different statistical definitions of photometric redshift change the measurement signal.

%%%%
In the analysis we divide the redshift range $0.2<z<1.4$ into different numbers of bins, in order to consider different bin configurations. As a result, a comparison between individual redshift bins for different bin configurations may be considering different redshift regions for each configuration. For this reason, we show here clustering results for bins with different widths but symmetrical about a given central redshift. In Figure \ref{fig:fig5} we present the the ratios of the photometric redshift clustering with respect to the true redshift clustering, for different statistical estimators and a redshift bin centered at $z=1$. We considered four different bin widths. 

In the left panels, we show the results for a broad bin of $\Delta z=0.3$ in the redshift range $0.85<z<1.15$. As expected, for all algorithms the amplitude of the clustering of the photometric sample is smaller than the case with the true redshift sample, due to the photo-z random dispersion that is well understood 
by a broader $n(z)$ for photometric samples.  Notice that the clustering measurements when using the mode, $\hat{z}$, and mean, $\bar{z}$,
estimators are similar, evaluating the ratios with respect to the true clustering. 
These small differences are due to the fact that each selection produces a different $n(z)$ when individual PDFs are not symmetric, like the one shown in Figure \ref{fig:fig1}.  

In the bottom panels we show the clustering ratios with respect to true clustering, and a comparison with the red dashed line that corresponds to the ratio for $\hat{z}$, of the clustering signals when using PDF weights,  
for different probability thresholds. This allows us to both clean our sample, as if using a quality cut, and sample  
thinner radial profiles. The thresholds considered are $p_{threshold}={0.0,0.1}$.  The signal depends on the cut on the selection of 
weighted galaxies. The stronger the cut, the cleaner the sample and the the clustering amplitude increases, as well as the intrinsic $n(z)$. But this may also bias our results as we may be changing the average galaxy types of the sample, as discussed in \pcite{marti14}. Here we considered a low threshold but non-negligible to compare with the full PDF case.
...
true redshift clustering increases because the signal is proportional to $n(z)^2$, as explained by Equation (\ref{eq:wtheta}), and in the top hat case, this means that it is inversely proportional to $(\Delta z)^2$. The bin considered in this case for 
galaxy selection is  $0.9<z<1.1$. We see that the differences between the mode and the mean are bigger than in the previous case with $\Delta z = 0.3$. This may be produced by the fact that when we consider bins much bigger than the intrinsic separations the differences between photo-z single statistic estimators are smaller.
As we extend the comparison to smaller widths, $\Delta z=0.15$ ($0.925<z<1.075$), the ratio between the angular clustering signal for the photometric samples and the sample with true redshift becomes smaller, in agreement with the trend we saw before,  as $w(\theta)\propto (1/\Delta z)^2$ for true redshift clustering, while the photo-z dispersion keeps the corresponding signal diluted in the radial direction.
...
The case with $\Delta z=0.1$ ($0.95<z<1.05$) shows the same trend than the previous cases with bigger bin widths. The ratio of the photometric redshift signals to the true redshift signal keeps getting smaller. The results with mean and mode estimators tend to converge as the bin width approaches the photo-z dispersion error. 
...
\caption{We present the evolution of the clustering amplitude at $\theta=0.1$, for a redshift bin centered at $z=0.5$, with bin width, $\Delta z$. We show the evolution for the true redshift 
case and for the photometric redshift estimators considered in order to see when the clustering signal saturates with bin width.} %We found that bins smaller than $\Delta z=0.15$ the 
...
\end{tabular}
\caption{\emph{Bias evolution for different redshift bin configurations:} We show the evolution with redshift of the linear galaxy bias, when dividing the full range in redshift bins. We present results for the different bin widths and the different photometric redshift estimators described in section \ref{sec:estimators}. We show the spectroscopic redshift results (blue shadow), mean photo-z (black star), mode photo-z (red triangle) and photo-z PDF weights (green cross). Notice that the spectroscopic sample in each bin is different than the corresponding photo-z sample in the same bin and we cannot directly compare them. The x-axis position is given by the mean redshift in each bin according to the $n(z)$, given by PDF stacking.}
...
\end{tabular}
\caption{\emph{Relative bias between PDF stacking and true redshift distributions:} We show the relative differences in the galaxy bias measurements when measuring the redshift distribution using PDF stacking with respect to the bias measurements given by the true redshift distribution. This is done for the three photo-z selection methods and the four redshift bin widths considered in this paper. Notice that for each method we are using the sample photometric sample in each bin but we are fitting the galaxy bias using different redshift distributions.}
...
\end{center}
\end{figure*}
We show in Figure \ref{fig:fig5b} the clustering amplitude evolution at $\theta=0.1$ for the angular correlations shown in Figure \ref{fig:fig5}. For photometric redshift estimators the clustering signal increases by about $50\%$ when increasing the numbers of bins by a factor of 4. Therefore, increasing the number of bins is not an efficient process for any photo-z estimator, from the point of view of clustering measurement, especially for bins with a width smaller than $\Delta z=0.15$, which is twice the mean photo-z error, as shown in Figure \ref{fig:fig3}.
...
using different photo-$z$ estimators affects the information on 
the linear galaxy bias $b_g$, defined in Equation (\ref{eq:bias}). Fitting the galaxy bias or any cosmological parameter can help us to calibrate the 
effect of photo-z estimators. We only used angular auto-correlation 
functions and one galaxy bias parameter per redshift bin. The total redshift range considered is $0.2<z<1.4$ and all the bins have the same width for 
each configuration. The angular range considered was set to cover the co-moving coordinates range $10 h^{-1}Mpc<r<60 h^{-1}Mpc$ which corresponds to different angular ranges in each redshift bin. The minimum scale was selected by testing at which scale the linear growth model for the spatial correlation departs from a non-linear model. Notice that this is a conservative cut, compared with the cuts used in \cite{crocce15}. This corresponds to $\theta_{min}=0.8$ degrees at the lowest redshift and $\theta_{min}=0.19$ degrees at highest redshift bin. 

The comparison between the different photo-z selection methods is done by comparing each galaxy bias measurement with the true result, which is determined by using the true redshift distribution of the selected galaxies, in order to do a fair comparison.  But first of all, we show in Figure \ref{fig:fig9} the photometric redshift measurements and the measurement done by selecting galaxies according to the spectroscopic redshift. Notice that the spectroscopic sample in each bin is different than the photometric samples considered, but this way we can study how the photo-z survey measurements compare with an spectroscopic one. 

In panel {\it a} of  Figure \ref{fig:fig9} we show the evolution of galaxy bias for the broad $\Delta z=0.3$ bin configuration. We only show results for true redshift, $z_s$, mode redshift, $\hat{z}$, mean redshift, $\bar{z}$ and the PDF weighted samples for clarity. We do not show here the results when applying quality cuts. The measurements are similar and the slightly different values for the different estimators are within statistical error bars. This is reasonable as we considered broad redshift bins and the differences between different photometric samples redshift distributions are small. 

The same trend is observed when considering $N_z=6$ redshift bins, panel {\it b} on Figure \ref{fig:fig9}. This case corresponds to bins with $\Delta z = 0.2$ width, which is larger than twice the photo-z dispersion and therefore photometric redshift effects are still not the biggest issue. The evolution of linear galaxy bias with redshift reseambles  the one shown in Figure 17 of \pcite{crocce15} for a MICECATv2.0 \cite{carretero15} sample, as the ratio $b_g(z=1.1)/b_g(z=0.3)$ is similar for both simulations.
...
We show in the lower left plot in Figure \ref{fig:fig9} the measurement of the bias for the different redshift estimators when considering $N_z=8$ redshift bins. We observe bigger differences between the 
case when using photometric redshifts and the case when the bias was obtained using spectroscopic redshifts, compared with the previous cases with larger bin widths. %This may be produced by the fact that when considering stacked PDFs, we are including also the tails of the distribution. The smaller the bin width, the bigger the effect of the tails on the $n(z)$.

Finally, we show in the bottom right panel on Figure \ref{fig:fig9} the bias evolution when using $12$ bins of width $\Delta z=0.1$. The differences between the photo-z galaxy bias results from the spectroscopic are larger than for the previous cases with broader bin configurations. The closest result to the spectroscopic results value of the bias is obtained when using the full PDF information $(p_{threshold}=0$), especially at intermediate redshifts.


We show in Figure \ref{fig:fig10} the measurement bias between the method that uses PDF stacking to estimate n(z) and the true value, given by the n(z) measured directly from the simulation true redshift of the photometric sample. We show in the top left panel the relative differences when considering $\Delta z=0.3$ for the three methods and we found small deviations with respect to the true results and not big differences between the different selection techniques. The observed differences exist because 
the PDF stacking technique is not perfectly reconstructing the true n(z) of the population sample in tomographic bins. In this case the differences are small because the bin width is broad and photo-z systematics in the n(z) are smaller.  

When we decrease the bin width to $\Delta z = 0.2$, the differences grow, as shown in the top right panel of Figure \ref{fig:fig10}. The three methods are still producing similar results and because the bins are still too broad the zero relative bias is within the error bars. 

When the configuration consists on bins with $\Delta z=0.15$ widths differences start to become more relevant and also the PDF weighting method becomes to differ from the single point estimate estimators. 

For the narrowest bin width configuration considered, $\Delta z = 0.1$ the differences at intermediate redshifts are larger than $5\%$ for single point estimators, whereas for PDF weighted galaxy samples these differences are around $3\%$. We include a table in Appendix \ref{sec:ap2} showing all the galaxy bias measurements and relative differences with the true results. 

In order to summarize and quantify these results, we show in Figure \ref{fig:fig13} the mean value of the mean absolute deviation for each selection method and each bin configuration with respect to the true result. 
\begin{figure}
\begin{center}
...
\caption{Evolution with redshift bin width of the percentual mean absolute deviation with the true result for the different photometric estimators. We shifted the x-axis values artificially in order to show clearly the results from different measures. We found an accumulated measurement bias when using 
photo-z redshifts. This deviation is smaller when using PDF weights in the clustering measurements, especially for the narrower bin configurations.}
\label{fig:fig13}
\end{center}
\end{figure}
We found that for the largest bin widths the differences are around $1\%$ and similar for the three photo-z selection methods (mean, mode, pdf weighting) but for the narrower bin the deviation when we consider summary statistics is around $5\%$ while it is $3\%$ when using the photo-z PDF galaxy weighting method proposed here.


\subsection{Reducing the redshift bin catalogue size}
Using full PDF information in galaxy clustering produces less biased measurements than point estimate photo-z methods but increases the amount of information 
that we have to store in each redshift bin galaxy sample. We studied how a Monte Carlo sampling of the PDF, in order to define a point estimate that encloses more PDF than the mean or the mode, or applying a threshold cut based on the amount of PDF in each bin compare with the full PDF inclusion method.

\subsubsection{Monte Carlo sampling redshift}
We extended the analysis to Monte Carlo sampling redshift, $z_{MC}$, in order to assign a redshift value based on the cumulative distribution function of each galaxy and then select them in the different redshift bins according to $z_{MC}$. In Figure \ref{fig:fig11b} we show the best fit galaxy bias for galaxies selected according to  $z_{MC}$ in $N_z=8$ redshift bins of $\Delta z = 0.15$.
\begin{figure}
\begin{center}
%\includegraphics[trim = 0cm 0cm 0cm 0cm, width=0.48\textwidth]{../Plots/Fig_bias_nz6__full_displacement.pdf}
...
\caption{\emph{Bias evolution for Monte Carlo sampling redshift shells of width $\Delta z = 0.15$:}  Bias measurement in $8$ redshift bins for Monte Carlo sampling redshifts (orange). We consider the cases with $n(z)$ estimated using PDF stacking and the true $n(z)$. We also compare the galaxy bias fitting with the PDF weights results from figure \ref{fig:fig9} (green). The standard performance of Monte Carlo redshifts is similar to the results given by PDF weights.}
\label{fig:fig11b}
\end{center}
\end{figure}
We observe that in this case the results are similar to the results given by PDF weights (taken from Figure \ref{fig:fig9} c), both when using PDF sampling or true redshift distributions. This is expected as we are using the probabilistic information to determine the Monte Carlo sampling redshifts. %This method can be considered in real surveys in order to define galaxy catalogues used to measure the correlations but we would still need the full PDF catalogue in order to estimate PDF or perform any other photometric redshift tests.
...
The effect of sparse PDFs with multiple peaks can introduce significant noise in our PDF weighting scheme. 
...
order to select galaxies in the different bins. The effect is a combination of a quality cut but also a cut on galaxies that are not in the bin but 
whose tails are inside the bin, making the maps bigger. In figure \ref{fig:figqual} we show a comparison between the a photo-z sample selected 
according to full PDFs for 
a configuration with bin width $\Delta z=0.15$ and a a sample selected by applying a threshold to the PDF weights of $f_z>0.1$.
\begin{figure}
\begin{center}
%\includegraphics[trim = 0cm 0cm 0cm 0cm, width=0.48\textwidth]{../Plots/Fig_bias_nz6__full_displacement.pdf}
...
\caption{\emph{PDF threshold cuts:} In this Figure study the effect of applying a threshold cut on the selection process with photo-z PDF weights for 
a the configuration with a redshift bin width of $\Delta z=0.15$. Both samples produce similar galaxy bias measurements.}
\label{fig:figqual}
\end{center}
\end{figure}
We found that the results are similar, enhancing the idea of applying threshold cuts to reduce the size of each galaxy 
density map per pixel, although cuts to a sample have to be applied carefully in order to 
avoid introducing selection biases to the sample, see \pcite{marti14}. We can observe that in Figure \ref{fig:figqual} as the true results for both samples are not exactly equivalent, 
A detailed study the quality cuts using PDF information outside the scope of this paper. 
...
\caption{\emph{Training set sample variance:} Relative differences between the amplitude of the theoretical dark matter angular correlations when we consider the spectroscopic n(z) of the training set or the mean n(z) for ten samples with the same number of spectroscopic objects than the training set distributed on the catalogue area. We considered the full redshift range $0<z<2$ and a redshift bin $0.3<z<0.5$. We found a relative bias in the angular range $0.1<\theta<1$ smaller than $2\%$, that propagates to an error smaller than $1\%$ for the galaxy bias which is lower than the observed effect due to photo-z definitions.} 
\label{fig:fig15}
\end{center}
\end{figure}
We show in Figure {\ref{fig:fig15}} the relative difference between theoretical angular correlations, computed using Equation (\ref{eq:wtheta}), when we used the training set n(z) (red line in Figure \ref{fig:fig2}) and the mean n(z) of ten different random samples extracted from the catalogue with the same number of galaxies than the training set, which is similar to the blue solid line in Figure \ref{fig:fig2}.  We found relative differences smaller than $2\%$ in the redshift range $0.1<\theta<1$. This implies a relative difference smaller than $1\%$ for the galaxy bias, which is lower than the differences observed according to the use of different photo-z estimators. 

\subsubsection{True redshift distribution reconstruction}
As observed in section \ref{sec:res3}, the main difference between the galaxy bias measurement and the true result is due to the failure to recover the true redshift. distribution $n(z)$. As an example, in Figure \ref{fig:fignzrecgauss} we show the difference between the true redshift distribution (blue line) and the PDF stacking PDF (green dashed line) for  galaxies selected with mean redshift within $0.65<z<0.8$.  
\begin{figure}
\begin{center}
\includegraphics[trim = 0cm 0cm 0cm 0cm, width=0.48\textwidth]{Figures_paper/Gaussian_error_pdf_paper_3.pdf}
\caption{\emph{Redshift distribution reconstruction:} In this Figure, we show the PDF stacking n(z) (dashed green) and the true redshift distribution (blue) obtained by selecting galaxies with mean photometric redshift in the redshift bin $0.65<z<0.8$. The red region covers the space between the n(z) obtained by stacking gaussian PDFs for the galaxy sample with standard deviation within $\sigma_{gauss}=0.03-0.1$. The differences between redshift distributions are contained within the photo-z error.}
...
In Appendix \ref{sec:ap3}, we show differences between the n(z) obtained through PDF stacking of galaxies selected in redshift shells according to the mean redshift and the true distribution of the same sample for the different bin configurations and different redshift ranges.
...
We studied how the angular galaxy clustering obtained from photometric populations depends on the different statistical estimators used 
for the sample selection in redshift bins. The estimators that we have considered are the mean and the mode of the photo-$z$ PDF. 
We found differences between the different 
estimators as they define different galaxy samples in each top hat photometric redshift bin. Therefore, the clustering signal is different when 
using the mean or the mode photometric redshift. 

We also included the full PDF information in the clustering analysis. We did that by weighting each galaxy according to the 
integrated probability of the galaxy in each redshift bin. The clustering signal is smaller than the clustering signal of single point estimates samples. If we apply a selection  threshold cut given by $p_{threshold}=0.1$ the signal amplitude increases. This is explained by the fact that when we consider a larger threshold the corresponding $n(z)$ is narrower than when considering all the galaxies with non negligible weights in a redshift bin but also we may be sampling different type of galaxies as we use only galaxies with higher probability to lie within the bin. 

We extended the comparison between the different definitions of photometric redshift to a cosmological parameter estimation analysis by measuring the linear galaxy bias in different redshift bins. We also found that, in general, the photo-z estimators produce similar results, especially when considering broad bins. We found that there is a relative bias with respect to the true galaxy bias results because the PDF stacking redshift distributions in each bin differ from the true redshift distributions. For narrow bins, the selection method given by PDF weights produces less biased differences with respect to the true results. The mean deviation for a bin configuration with width $\Delta z=0.1$ is a $3\%$ when using PDF weights while it is a $5\%$ when using summary or single point estimate statistics. Therefore the use of photo-z PDF weights to select galaxies in tomographic redshift bins in order to measure the galaxy clustering in a photometric survey is more optimal than using single estimates.   
We can also use the same methodology used in this paper to calibrate the effect of the photo-z bin galaxy selection in the 
model parameters with simulations mimicking the real data catalogues. This applies to other photo-z methods that estimate PDFs 
as the have similar behaviours \cite{sanchez14,bonnet15,leistedt2015}. 

Creating maps with PDF weights involves bigger amounts of data than catalogues of galaxies selected 
by redshift. One way to reduce the amount of data is to apply a cautious PDF quality cut by using a threshold when considering PDF weights. We found similar results 
to the full PDF results although any cut on a sample has to be previously tested.
Another way to reduce the size of the catalogues but still including to a certain level PDF information is by using Monte Carlo Sampling point estimates. 
We found that Monte Carlo Sampling estimators produce similar results to PDF weight results.

The effect of choosing different training sets on the galaxy bias measurement is smaller than $1\%$. which is lower than the effects due to photo-z definitions measured in this paper. Also, the differences between PDF stacking redshift distributions and the true redshift distributions are within the photo-z errors.
...
With photometric surveys we can access a larger number of galaxies in less time than with spectroscopic surveys. However, the lack of true redshifts reduces the amount of 
radial information on the survey, as photometric redshift are produced from multi-band imaging.  

We need to set a statistical definition of photometric redshift in order to select photometric galaxies in tomographic redshift bins. The search for an optimal definition 
is the main goal of this paper. The core analysis of this paper consisted on defining a new photo-z selection method that includes the full photo-z PDF information by weighting each galaxy in the redshift bin with the probability to be in that bin and compare it with two methods based on the single point estimates as the mean or the mode redshift.

We found, using mock galaxy catalogues and machine learning photo-z code, that if we use single point estimates, like the mode or the mean, there is an offset on the galaxy bias measurements, which are obtained by measuring redshift distributions by stacking the PDF of the selected galaxies, compared with the true results, defined by the true redshift distribution of the same galaxies. This shift should be taken into account when considering similar Large Scale Structure analysis with photometric surveys. This corrective effect can be estimated applying a similar method to measuring the offset in the determination of cosmological parameters using simulations in similar conditions to the expected photometric data. In our case, we used the galaxy bias as the metric to test photo-z definitions and we found that, for single point estimate redshifts, the cumulative deviation is a $5\%$ for a bin configuration with width $\Delta z= 0.1$.

Results are closer to the truth if we count galaxies according to PDF weights in each redshift bin, being the differences a $3\%$ in the $\Delta z=0.1$ 
configuration. Therefore, and especially for narrow photometric top hat bins, PDF weighting is more optimal than using 
single point estimates or summary statistic photometric redshifts.



\section*{Acknowledgments}
JA and JJT acknowledge support from U.S. Department of Energy Grant No, DE-SC0009932. ISN would like to thank the Spanish Ministry of Economy and Competitiveness (MINECO) for funding support through grant FPA2013-47986-C3-2-P. RJB acknowledges support from the National Science Foundation Grant No. AST-1313415. RJB has been supported in part by the Center for Advanced Studies at the University of Illinois. This work also used resources from the Extreme Science and Engineering Discovery Environment (XSEDE), which is supported by National Science Foundation grant number OCI-1053575. We want to acknowledge M. Becker and R. Wechsler for their input regarding the simulation catalogue. We thank all the useful discussions  with DES members and especially the useful discussions with the internal reviewers. This paper has gone through internal review by the DES collaboration. The DES publication number for this article is XXXX. The Fermilab pre-print number is XXX.
...
In order to check the robustness PDFs that we used in the paper, we estimated the distribution of the 
...
\caption{\emph{Photometric standardized error for the mean}: We show the the standardized error using the mean of 
each individual galaxy. The solid red line corresponds to the best fit gaussian with mean $\mu$ and error $\sigma_G$} 
\label{fig:figap1}
\end{center}
\end{figure}
In Figure \ref{fig:figap1} we show the results for the mean redshifts. We observe that the simple error estimate is close to 
the the unbiased estimate ($\mu=0$, $\sigma_G=1$). If we consider the mode redshift, we see in Figure \ref{fig:figap2} that 
the distribution of modes tends to be more concentrated than the distribution of means.
\begin{figure}
\begin{center}
%\includegraphics[trim = 0cm 0cm 0cm 0cm, width=0.48\textwidth]{../Plots/Fig_bias_nz6__full_displacement.pdf}
\includegraphics[trim = 0cm 0cm 0cm 0cm, width=0.48\textwidth]{Figures_paper/Fig_ap_2.pdf}
\caption{\emph{Photometric standardized error for the mode}:  Photometric standardized error for the mode and 
the comparison with the best fit gaussian. As modes are defined by the peak of each PDF, the distribution tends 
...
by estimating the number of galaxies with photometric redshifts inside $1-\sigma$ and $2-\sigma$ levels, shown in Table \ref{table:tableap1}.
\begin{table}
{\center
\begin{tabular}{|c|c|c|}
\hline
...
In table \ref{table:table1}, we show the galaxy bias fits for the different bin configurations and the the three photometric redshift methods (mean, mode, PDF) considered in the paper to 
select galaxy in tomographic redshift bins, when considering PDF stacking redshift distributions. We also show the goodness of fit for each fit and the relative differences with the true measurements.
\begin{table*}
{\center
...
As shown in section \ref{sec:res3}, the galaxy bias measurements obtained from n(z) given by PDF stacking differ with the true measurements. This is caused by 
...
for galaxies selected according to the mean redshift on a given set of top hat redshift bins at low redshift. We note that the tails of the PDF stacking n(z) are longer than
\begin{figure*}
\begin{center}
%\includegraphics[trim = 0cm 0cm 0cm 0cm, width=0.48\textwidth]{../Plots/Fig_bias_nz6__full_displacement.pdf}
...
\caption{\emph{Low redshift}:  Redshift distributions for the true redshift distribution of galaxies selected according to mean photo-z redshift and the PDF stacking 
redshift distribution for the the same galaxy sample. We considered low redshift samples.} 
...
\caption{\emph{Intermediate redshift}: Comparison between the true redshift distribution of galaxies selected according to mean photo-z redshift and the PDF stacking 
redshift distribution for the same galaxies.} 
%\caption{Redshift distributions of the photo-z sample, true redshift distribution of galaxies selected according to mean photo-z redshift and the PDF stacking 
%redshift distribution for the same galaxies.} 
\
...
\caption{\emph{High redshift}: True redshift distribution of galaxies selected according to mean photo-z redshift compared with the distribution given by the PDF stacking of 
the same sample.} 
%\caption{Redshift distributions of the photo-z sample, true redshift distribution of galaxies selected according to mean photo-z redshift and the PDF stacking 
%redshift distribution for the same galaxies.} 
\
